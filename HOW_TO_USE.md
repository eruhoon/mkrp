...
1. **Python 2 ➔ 3 스탯 비교 및 screen 호환성**:
   - Ren'Py 7 구버전 게임의 스탯/호감도 비교 구문(`__cmp__`, `< None`) 충돌 및 `renpy.has_screen` 속성 누락 에러는 `zz_handheld_patch.rpy`가 자동으로 감지하여 완벽히 방어합니다.
2. **저사양 RAM 기기 메모리 관리**:
   - `zz_handheld_patch.rpy`가 부팅 시 `/proc/meminfo`를 확인하여 1GB 기기(160MB 캐시, 보수적 프리딕트)부터 4GB 기기(512MB 캐시)까지 자동으로 스케일링하여 메모리 부족(OOM) 강제 종료를 방지합니다.
